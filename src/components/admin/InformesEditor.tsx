...
type InformeStatus = "draft" | "published" | "featured";

interface InformesEditorProps {
    informeId?: string;
}

export default function InformesEditor({ informeId }: InformesEditorProps) {
    ...
    const [formData, setFormData] = useState({
        ...
        excerpt: "",
        imageUrl: "",
        status: "draft" as InformeStatus,
    });

    // Fetch Informe data if editing
    useEffect(() => {
        if (informeId) {
            const fetchInforme = async () => {
                const docRef = doc(db, "informes", informeId);
                ...
                if (docSnap.exists()) {
                    ...
                    setFormData({
                        ...
                        status: (data.status as InformeStatus) || "draft",
                    });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!informeId) {
            informeData.createdAt = serverTimestamp();
        }

        try {
            if (informeId) {
                await updateDoc(doc(db, "informes", informeId), informeData);
            } else {
    ...
    return (
        <form onSubmit={handleSubmit} className="max-w-6xl mx-auto space-y-12">
            <div className="flex items-center justify-between">
                <Link href="/admin/informes" className="flex items-center text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-primary transition-colors">
                ...
                <div className="bg-white rounded-[3rem] p-10 shadow-sm border border-gray-100 space-y-10">
                    <div className="space-y-6">
                        <div className="flex items-center space-x-3 text-accent mb-2">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest">Información del Informe</span>
                        </div>
                        <input
                            type="text"
                            name="title"
                        ...
                        <div className="grid grid-cols-2 gap-6">
                            <div className="space-y-4">
                                <div className="flex items-center space-x-3 text-accent mb-2">
                                    <span className="text-[10px] font-black uppercase tracking-widest text-primary">Estado</span>
                                </div>
                                <select
                                    name="status"
                                    value={formData.status}
                                    onChange={handleChange}
                                    className="w-full bg-gray-50 rounded-2xl px-6 py-4 text-sm font-black uppercase tracking-widest outline-none border-2 border-transparent focus:border-accent transition-all shadow-inner appearance-none cursor-pointer"
                                >
                                    <option value="draft">Borrador</option>
                                    <option value="published">Publicado</option>
                                    <option value="featured">Destacado (Menú)</option>
                                </select>
                            </div>
                            <div className="space-y-4">
                                <div className="flex items-center space-x-3 text-accent mb-2">
                                ...
                                <input
                                    ...
                                    className="w-full bg-gray-50 rounded-2xl px-6 py-4 text-sm font-bold outline-none border-2 border-transparent focus:border-accent transition-all shadow-inner"
                                    required
                                />
                            </div>
                        </div>
